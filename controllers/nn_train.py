...
class Controller(BaseController):
    def __init__(self):
        ...
        self.state = torch.zeros((1, self.model.state_dim))
    ...
    def update(self, target_lataccel: float, current_lataccel: float, state, future_plan):
        ...
        future_plan = torch.cat([future_plan, torch.zeros((future_plan.shape[0], FUTURE_PLAN_LENGTH - future_plan.shape[1]))], dim=-1)
        inp = torch.cat((
